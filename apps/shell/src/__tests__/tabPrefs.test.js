...
describe("defaults", () => {
  ...
  it("leaves ZTS, Clarify and Runway visible, in APPS order", () => {
    expect(visibleTabs(null)).toEqual(["zts", "clarify", "runway"]);
  });
...
describe("toggleTab", () => {
  ...
  it("restores a re-shown tool to its original position, not to the end", () => {
    ...
    expect(visibleTabs(p)).toEqual(["zts", "clarify", "runway", "macro"]);
  });
...
describe("moveTab", () => {
  ...
  it("moves within the FULL order, so a hidden neighbour is not skipped", () => {
    ...
    expect(p.order).toEqual(["zts", "clarify", "macro", "runway", "looper", "business"]);
    expect(visibleTabs(p)).toEqual(["zts", "clarify", "runway"]); // visible order unchanged
  });
